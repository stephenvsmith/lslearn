...
A list with the estimated PDAG adjacency matrix (`amat`), separating
sets (`S`), the number of conditional independence tests used
(`NumTests`, `MBNumTests`), the nodes considered (`Nodes`, in 1-based R
numbering), timing information, the reference DAG used (`referenceDAG`),
the estimated Markov Blanket list (`mbList`, if applicable), which
orientation rules were used (`rules_used`), and, when `data` was
